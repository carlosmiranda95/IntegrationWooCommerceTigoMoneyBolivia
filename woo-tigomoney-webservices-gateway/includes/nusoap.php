...
class soap_transport_http extends nusoap_base {
	...
	* @access   private
	*/
	function connect($connection_timeout=0,$response_timeout=180){
...
//		}
	...
	* @access   public
	*/
	function send($data, $timeout=0, $response_timeout=180, $cookies=NULL) {
	...
	* @deprecated
	*/
	function sendHTTPS($data, $timeout=0, $response_timeout=180, $cookies) {
...
class wsdl extends nusoap_base {
     ...
     * @access public 
     */
    function wsdl($wsdl = '',$proxyhost=false,$proxyport=false,$proxyusername=false,$proxypassword=false,$timeout=0,$response_timeout=180,$curl_options=null,$use_curl=false){
		parent::nusoap_base();
...
class nusoap_client extends nusoap_base  {
	...
	var $timeout = 180;				// HTTP connection timeout
	var $response_timeout = 180;		// HTTP response timeout
	...
	* @access   public
	*/
	function nusoap_client($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false, $timeout = 180, $response_timeout = 180, $portName = ''){
		parent::nusoap_base();
	...
	* @access   private
	*/
	function send($msg, $soapaction = '', $timeout=180, $response_timeout=180) {
		$this->checkCookies();
		// detect transport
		switch(true){
			// http(s)
